    ...
    def post(self, request, slug):
        ...
        if existing_item:
            ...
            if total_quantity > product_size.stock:
                return JsonResponse({
                    'error': f'Cannot add {quantity} items. Only {product_size.stock - existing_item.quantity} more avalaible.'
                }, status = 400)
    ...
    def post(self, request, item_id):
        ...
        if quantity == 0:
            ...
            if quantity > cart_item.product_size.stock:
                return JsonResponse({
                    'error': f"Only {cart_item.product_size.stock} items avalaible"
                }, status = 400)
